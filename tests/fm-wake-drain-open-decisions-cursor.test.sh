...
test_terminal_line_keeps_decisions_open_in_cached_drains() {
  local dir state status cursor out kind terminal expected ident size span pass_number
    ...
    for terminal in 'done' failed; do
      ...
      for pass_number in 1 2; do
        if [ "$pass_number" = 2 ]; then
          # A cursor persisted by the kind-scoped fold, which dropped every
          # decision at a ship's or scout's terminal line, must be refolded.
          ident=$(sed -n 's/^ident=//p' "$cursor")
          size=$(LC_ALL=C wc -c < "$status" | tr -d '[:space:]')
          printf 'version=8:%s\noffset=%s\nident=%s\n' "$kind" "$size" "$ident" > "$cursor"
        fi
        FM_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$dir/drain.err" || fail "$kind terminal drain failed"
        assert_contains "$(cat "$out")" 'task [key=access] blocked: waiting' "$kind blocker must survive $terminal and cache migration"
        bash -c '. "$1"; [ "$(status_open_decisions "$2")" = "$3" ] && [ "$(status_open_decisions_incremental "$2")" = "$3" ] && [ "$(status_key_closing_verb "$2" access)" = blocked ]' \
      ...
      assert_contains "$span" 'blocked [key=access]: waiting' "$kind opening must remain actionable in a captured span"
  ...
  pass "terminal lines keep decisions open in whole-file reads, incremental drains, old caches, and captured spans"
}

test_task_kind_never_changes_folded_decisions() {
  ...
  for kind in unknown ship secondmate scout; do
    ...
    bash -c '. "$1"; [ "$(status_open_decisions_incremental "$2")" = "$3" ] && [ "$(status_open_decisions "$2")" = "$3" ]' \
      _ "$ROOT/bin/fm-classify-lib.sh" "$status" "$expected" \
      || fail "folded decisions changed with $kind metadata"
  done
  pass "folded decisions are the same for every task kind"
}
