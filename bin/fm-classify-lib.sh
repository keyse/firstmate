...
# one supplies it; the latest recognized event stands when none applies.
...
status_current_line() {  # <status-file> [<kind>]
  local f=$1 kind line resolve held since='' verb key note current=''
  if [ -f "$f" ] && [ -r "$f" ] && [ ! -L "$f" ]; then
    ...
    while IFS= read -r line || [ -n "$line" ]; do
      ...
      case "$verb" in
          ...
          case "$kind:$line" in ship:*:*|scout:*:*) since='' ;; esac
          ;;
        needs-decision|blocked|"$resolve"|"$held")
...
# 9: a ship's or scout's done/failed line no longer closes any decision, so the
